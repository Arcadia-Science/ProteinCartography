...
The requests that `blast_utils` makes to NCBI are covered separately, by the unit tests in `ProteinCartography/tests/test_blast_utils.py`. Those tests mock every HTTP response and fake the clock, so they cover the polling and timeout behavior without sleeping or touching the network.
