...
The one external call that is not mocked with the shared `requests` mock is the blast search. It is stubbed out instead: setting the `PROTEINCARTOGRAPHY_BLAST_STUB_RESULTS_FILEPATH` env variable to the path of a canned blast results file makes `blast_utils.run_blast` copy that file to its output path instead of querying NCBI (which takes 10-40 minutes at best, because the search queues on NCBI's public servers, and can take hours when that queue is busy). The tests set this env variable in the `set_env_variables` fixture; it should never be set in production.

The requests that `blast_utils` makes to NCBI are covered separately, by the unit tests in `ProteinCartography/tests/test_blast_utils.py`. Those tests mock every HTTP response and fake the clock, so they cover the polling and timeout behavior without sleeping or touching the network. They also convert a real (if trimmed) NCBI response, committed as `api_response_content/blast.ncbi.nlm.nih.gov_Blast.cgi_JSON2_S`, and check the rows it produces against the results file from a real `blastp -remote` run, so that the parser is not only tested against hand-written mocks. The timeout budget shared by the retry attempts is covered by `ProteinCartography/tests/test_run_blast.py`.
