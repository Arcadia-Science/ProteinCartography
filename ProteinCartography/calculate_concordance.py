...
def parse_args():
    ...
    parser.add_argument(
        "-t", "--tmscore-file", required=True, help="A TM-score-based distances TSV."
    )
    parser.add_argument(
        "-f",
        "--fident-file",
        required=True,
        help="A fraction sequence identity-based distances TSV.",
    )
    parser.add_argument(
        "-p", "--protid", required=True, help="Unique identifier of the input protein."
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        help="Returns a .tsv file of concordance features as output.",
    )
    parser.add_argument(
        "-m",
        "--method",
        default="linear",
        help="The method to use to calculate concordance/ divergence.",
    )
    parser.add_argument(
        "-e",
        "--evalue-maximum",
        default="0.001",
        help="Maximum evalue cutoff. Default = 0.001",
    )
    parser.add_argument(
        "-T",
        "--tmscore-minimum",
        default="0",
        help="Minimum TM-score cutoff. Default = 0",
    )
...
def calculate_concordance(
    ...
    joint_df[f"concordance_v_{protid}"] = joint_df.apply(
        lambda x: concordance_fxn(x[f"TMscore_v_{protid}"], x[f"fident_v_{protid}"]),
        axis=1,
    )
...
def main():
    ...
    calculate_concordance(
        ...
        output_file,
        method=method,
        evalue_maximum=evalue_maximum,
        tmscore_minimum=tmscore_minimum,
    )
