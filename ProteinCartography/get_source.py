...
def parse_args():
    ...
    parser.add_argument("-i", "--input", required = True, help = 'Input filepath containing list of protids as index. Usually pivoted distance matrix.')
    parser.add_argument("-f", "--hit-files", nargs = '+', required = True, help = 'List of filepaths to parse source info.')
    parser.add_argument("-o", "--output", required = True, help = 'Destination filepath.')
    parser.add_argument("-k", "--keyids", nargs = '+', default = [], help = 'Key ids (usually protids of input proteins) used to determine the source of a protid.\nUseful when there are multiple input proteins.')
